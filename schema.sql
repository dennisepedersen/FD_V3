...
CREATE INDEX ix_labs_idea_status_updated ON labs_idea (status, updated_at DESC);
CREATE INDEX ix_labs_idea_module_updated ON labs_idea (module_key, updated_at DESC);
...
CREATE INDEX ix_labs_analysis_idea_created ON labs_analysis (idea_id, created_at DESC);
CREATE INDEX ix_labs_analysis_recommendation_score ON labs_analysis (recommendation, score DESC);
...
CREATE INDEX ix_labs_attachment_idea_created ON labs_attachment (idea_id, created_at DESC);
CREATE INDEX ix_labs_attachment_active ON labs_attachment (idea_id) WHERE archived_at IS NULL;

-- ============================================================================
-- 24) labs_idea_history
-- ============================================================================

CREATE TABLE labs_idea_history (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  idea_id uuid NOT NULL,
  event_type text NOT NULL,
...
CREATE INDEX ix_labs_idea_history_idea_created ON labs_idea_history (idea_id, created_at DESC);
