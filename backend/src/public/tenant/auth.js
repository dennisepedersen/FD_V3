(function () {
  ...
  async function initAcceptInvitePage() {
    ...
    try {
      ...
      const expiry = invitation.expires_at ? ` Linket udl�ber ${new Date(invitation.expires_at).toLocaleString("da-DK")}.` : "";
      if (intro) intro.textContent = `V�lg en adgangskode til din Fielddesk-konto.${emailHint}${expiry}`;
    ...
    form.addEventListener("submit", async (event) => {
      ...
      if (nextPassword.length < 10) {
        setStatus("Adgangskoden skal v�re mindst 10 tegn.", "error");
        return;
      }
  ...
  async function initAppPage() {
    ...
    function renderTenantAdminSyncStatus() {
      ...
      if (!sync) {
        tenantAdminSyncStatus.textContent = "Indl�ser sync-status...";
        return;
      }
      if (!configured) {
    ...
    function getTenantAdminInviteDisabledReason(user) {
      if (!user || !(user.tenant_user_id || user.fitter_row_id)) return "Kr�ver en tenant user- eller fitter-r�kke";
    ...
    function renderTenantAdminUsers() {
      ...
      setText(tenantAdminUsersMeta, state.tenantAdmin.usersLoading ? "Indl�ser medarbejdere..." : (users.length === 1 ? "1 medarbejder." : `${users.length} medarbejdere.`));
      ...
      users.forEach((user) => {
        ...
        const lastSent = user && (user.invitation_sent_at || user.last_invited_at)
          ...
          : "";
        const expires = user && user.invitation_expires_at ? ` � Udl�ber: ${formatTenantAdminDate(user.invitation_expires_at)}` : "";
    ...
    async function submitTenantAdminUserCreate(event) {
      ...
      if (!name || !email) {
        setText(tenantAdminUserCreateStatus, "Navn og email er p�kr�vet.");
        return;
      }
    ...
    async function syncTenantAdminFitters() {
      ...
      setText(tenantAdminSyncStatus, "Sync k�es...");
      try {
        const response = await apiFetch("/api/tenant/admin/integrations/ekomplet/fitters/sync", { method: "POST" });
        setText(tenantAdminSyncStatus, response && response.reused ? "Sync k�rer allerede." : "Sync er sat i k�.");
