...
function mapWindow(row, { scopes = null, relevantAbsenceTypes = null, asOfDate = todayDate() } = {}) {
  ...
  const mapped = {
    ...
    vacation_day_exemption_quota: row.vacation_day_exemption_quota === null || row.vacation_day_exemption_quota === undefined ? 1 : Number(row.vacation_day_exemption_quota),
  ...
  try {
    return await withTransaction(async (client) => {
      ...
      const created = await repository.insertWindow(client, {
        ...
        vacationDayExemptionQuota: payload.vacationDayExemptionQuota,
