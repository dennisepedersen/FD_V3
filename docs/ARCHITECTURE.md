...
- PR2 adds request foundation tables. PR3 adds employee own request backend routes. PR4 adds notification/outbox foundation. PR5 adds manager pending/detail/approve/reject backend routes using snapshot-manager object scope, optimistic versioning, request events, audit, and employee notification/outbox side effects. Personnel manager access is object-scoped to `absence_request.assigned_manager_tenant_user_id` and is independent of system role; roles do not grant blanket manager access. PR6 adds approved-absence calendar feeds. PR7 adds special vacation window administration, deadline enforcement, and review overview under `/api/calendar/special-windows`. PR8 adds the first static tenant UI for employee requests, manager treatment, minimal agenda feeds, and special-window administration. Alternative-period workflow, full calendar UI, automatic mail sending, outbox apply, Render change, and production mutation remain outside PR8.
