...
- The employee flow uses read-only `POST /api/calendar/absence-requests/preflight` once type and date/time fields are complete, then creates or patches a draft, shows a confirmation summary, and submits with `Idempotency-Key`. Submit remains authoritative and revalidates the special-window rules. The UI supports `full_days` and `time_range`; `partial_day` remains hidden/deferred.
...
- Special vacation window UI lists, creates, edits, archives, and opens review overview through the PR7 `/api/calendar/special-windows` endpoints. Scope selectors use existing tenant-user/resource-group/type options instead of hardcoded users or groups. The create UI hides the technical key; backend create generates it when omitted. Empty absence-type scope means all active request types where `special_window_eligible = true`, and the UI has `Ryd valg` controls for the multiselect scopes.
