...
- Calendar / Resource Absence direct `resource_absences` API exposes full absence read/create only to `tenant_admin`; `project_leader` and `technician` are denied until masked visibility/resource-scope policy exists. PR3 absence-request API separately allows authenticated users to manage only their own `absence_request` rows through `absence_request:*_own` permissions. PR5 manager routes use the concrete request snapshot as object scope: a user may read/approve/reject only rows where `assigned_manager_tenant_user_id` equals the current active tenant user. System role and personnel manager status are separate; tenant admin, project leader, resource group manager, and imported fitter identities are not automatic approvers.
...
- Resource groups, members, and managers are tenant-isolated and Fielddesk-owned.
...
- Future absence request workflow must keep request, approved absence, calendar feed, notification state, and audit history separate. Private employee comments are visible to the employee and the concrete assigned approver through request object scope; broader review surfaces require explicit permission, and tenant admin is not automatically a private-comment reader. Manager decision text is stored in request event history and is visible through employee/assigned-manager request detail, but is not copied to audit metadata, calendar feeds, logs, or broad tenant surfaces. Personnel approval must be based on a Fielddesk-owned manager relation, not project responsible, resource group manager, tenant admin role, or imported `fitter` identity.
- PR2 absence request tables use composite tenant foreign keys and `tenant_user` identities for employees/managers. PR3 own-request routes derive employee from auth, reject client-controlled tenant/employee/manager/status fields, require optimistic versioning on mutations, and write events/audit in the same transaction. PR5 manager routes derive manager from auth, enforce snapshot-manager object scope, reject client-controlled manager/status/tenant fields, require optimistic versioning on approve/reject, and write request event, audit, notification, and outbox rows in one transaction. Manager object-scope is independent of system role: `technician`, `project_leader`, and `tenant_admin` can be personnel managers only for requests assigned to their own `tenant_user.id`, and none of those roles grants blanket managed access. The assigned manager can read the request's employee comment because the request row is scoped to that manager; `absence_request:read_private_comment` remains a separate permission for broader review surfaces and is not granted to tenant-admin by default. Rejection reasons are stored in request event reason and employee mail content, but not in audit metadata, notification payload/body, outbox payload, or route/processor logs. RLS remains a future defense-in-depth gap. PR7 special vacation window administration uses `absence_special_window:manage`; review overview can use explicit `absence_special_window:review` but still redacts private comments unless `absence_request:read_private_comment` is also granted. Tenant-scoped resource-group membership may include a request in a special window, but resource groups still do not grant personnel approval, project access, or private-comment visibility.
